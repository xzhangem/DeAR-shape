...
def SymmetricH2Matching(source,target,geod,F_init,vertex_edge_init,a0,a1,b1,c1,d1,a2,curvature_coeff,param):    
    ...
    energy = enr_match_H2_sym(VS, FS, FunS, VT, FT, FunT, F_sol, Fun_sol, B_sol, vertex_opposite_edge, weight_coef_dist_S=weight_coef_dist_S ,weight_coef_dist_T=weight_coef_dist_T, kernel_geom = kernel_geom, kernel_grass = kernel_grass, kernel_fun = kernel_fun, sig_geom = sig_geom, sig_grass = sig_grass, sig_fun = sig_fun,a0=a0,a1=a1,b1=b1,c1=c1,d1=d1,a2=a2,curvature_coeff=curvature_coeff)
...
def H2MultiRes(source,target,a0,a1,b1,c1,d1,a2,resolutions,paramlist,start=None, rotate = False, hodge_model_path=None):
    N=2
    if len(source) == 2:
        [VS, FS] = source
        [VT, FT] = target
    ...
    for j in range(0,iterations):
        ...
        if j == (iterations-1) or (j == 0):
            geod,ener,Dic=SymmetricH2Matching(sources[index],targets[index],geod,F0,vertex_edge_0,a0,a1,b1,c1,d1,a2,1,params)
        else:
            geod,ener,Dic=SymmetricH2Matching(sources[index],targets[index],geod,F0,vertex_edge_0,a0,a1,b1,c1,d1,a2,1,params)
        print(j,F0.shape)
        if time_steps>2:
...
def H2StandardIterative(source,target,a0,a1,b1,c1,d1,a2,paramlist,N=2):
    #N=2
    #[VS,FS]=source
    #[VT,FT]=target
    ...
    for j in range(0,iterations):
        ...
        #geod, ener, Dic = SymmetricH2Matching(source,target,geod,F0,a0,a1,b1,c1,d1,a2,params)
        print(j,F0.shape)
        if time_steps>2:
            geod=H2Midpoint(geod,time_steps,F0,a0,a1,b1,c1,d1,a2,params)
    print(iterations,F0.shape)            
