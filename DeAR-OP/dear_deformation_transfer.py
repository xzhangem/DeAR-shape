...
from dear_ivp import stepforward, DeARInitialValueProblem
from dear_param import DeARParameterized
...
def DeARParamDeformationTransfer(VS, VT, VNS, F,a0,a1,b1,c1,d1,a2, geod1_params, geod2_params):
    print("Calculating Deformation Geodesic")
    geod1,F0=DeARParameterized([VS,F],[VT,F],a0,a1,b1,c1,d1,a2,geod1_params)
    print("Calculating Transfer Geodesic")
    geod2,F0=DeARParameterized([VS,F],[VNS,F],a0,a1,b1,c1,d1,a2,geod2_params)
    ...
    Ngeod1,F_init=DeARInitialValueProblem(VNS,(N-1)*(X0.cpu().numpy()-VNS),N,a0,a1,b1,c1,d1,a2,F0)
    
    return geod1,geod2,Ngeod1,F

def DeARUnparamDeformationTransfer(source, target, new_source, a0,a1,b1,c1,d1,a2, geod1_params,geod2_params, time_step=2, parameterizedDeform=False, parameterizedTransfer=False, match_newsource=False):
    print("Calculating Deformation Geodesic")
    if(parameterizedDeform):
        geod1,F0=DeARMultiRes(source,target,a0,a1,b1,c1,d1,a2, time_step, geod1_params)
    else:
        geod1,F0=DeARMultiRes(source,target,a0,a1,b1,c1,d1,a2, time_step, geod1_params)
        
    print("Calculating Transfer Geodesic")
    if(parameterizedTransfer):
        geod2,F0=DeARMultiRes(source,new_source,a0,a1,b1,c1,d1,a2, time_step, geod2_params)
    else:
        geod2,F0=DeARMultiRes(source,new_source,a0,a1,b1,c1,d1,a2, time_step, geod2_params)
    ...
    N1=geod2.shape[0]
    print("Calculating Transfered Deformation")
    Ngeod1,F_1=DeARInitialValueProblem(geod2[N1-1], N*(X0.cpu().numpy()-geod2[N1-1]),N,a0,a1,b1,c1,d1,a2,F0)    
    
    if match_newsource:
        print("Calculating Transfered Deformation on New Source mesh structure")
        Ngeod2,F_2=DeARStandardIterative(new_source,[Ngeod1[N-1],F_1.cpu().numpy()],a0,a1,b1,c1,d1,a2,geod1_params)
