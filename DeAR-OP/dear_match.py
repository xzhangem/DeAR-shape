...
from enr.dear import *
from dear_param import DeARMidpoint
import numpy as np
import scipy
from scipy.optimize import minimize,fmin_l_bfgs_b
from enr.DDG import computeBoundary, vertexOppositeEdge
...
def SymmetricDeARMatching(source,target,geod,F_init,vertex_edge_init,a0,a1,b1,c1,d1,a2,curvature_coeff,param):    
    ...
    vertex_opposite_edge = torch.from_numpy(vertex_edge_init).to(dtype=torch.long, device=torchdeviceId)
    #print(F_sol.requires_grad)
    #print(vertex_opposite_edge.requires_grad)
    ...
    energy = enr_match_DeAR_sym(VS, FS, FunS, VT, FT, FunT, F_sol, Fun_sol, B_sol, vertex_opposite_edge, weight_coef_dist_S=weight_coef_dist_S ,weight_coef_dist_T=weight_coef_dist_T, kernel_geom = kernel_geom, kernel_grass = kernel_grass, kernel_fun = kernel_fun, sig_geom = sig_geom, sig_grass = sig_grass, sig_fun = sig_fun,a0=a0,a1=a1,b1=b1,c1=c1,d1=d1,a2=a2,curvature_coeff=curvature_coeff)
...
def StandardDeARMatching(source,target,geod,F_init,a0,a1,b1,c1,d1,a2,param):    
    sig_geom=param['sig_geom']
    ...
    # Define Energy and set parameters
    energy = enr_match_DeAR(VS, VT, FT, FunT, F_sol, Fun_sol, B_sol,weight_coef_dist_T=weight_coef_dist_T, kernel_geom = kernel_geom, kernel_grass = kernel_grass, kernel_fun = kernel_fun, sig_geom = sig_geom, sig_grass = sig_grass, sig_fun = sig_fun,a0=a0,a1=a1,b1=b1,c1=c1,d1=d1,a2=a2)
...
def DeARMultiRes(source,target,a0,a1,b1,c1,d1,a2,resolutions,paramlist, inter_state=2, start=None, rotate = False):
    N=inter_state
    [VS, FS] = source
    [VT, FT] = target
    sources = [[VS, FS]]
    ...
    if start!=None:
        ...
        F0=start[1]
        vertex_edge_0 = vertexOppositeEdge(F0)
        for i in range(0,N):
            geod[i,:,:]=start[0]
    ...
    for j in range(0,iterations):
        ...
        if j == (iterations-1) or (j == 0):
            geod,ener,Dic=SymmetricDeARMatching(sources[index],targets[index],geod,F0,vertex_edge_0,a0,a1,b1,c1,d1,a2,1,params)
        else:
            geod,ener,Dic=SymmetricDeARMatching(sources[index],targets[index],geod,F0,vertex_edge_0,a0,a1,b1,c1,d1,a2,1,params)
        print(j,F0.shape)
        if time_steps>2:
            geod=DeARMidpoint(geod,time_steps,F0,a0,a1,b1,c1,d1,a2,params)
        if tri_upsample:
            geod_sub=[]
            F_Sub=[]
            for i in range(0,N):
...
def DeARStandardIterative(source,target,a0,a1,b1,c1,d1,a2,paramlist,N=2):
    #N=2
    #[VS,FS]=source
    #[VT,FT]=target
    ...
    for j in range(0,iterations):
        ...
        geod=np.array(geod)
        [N,n,three]=geod.shape
        geod,ener,Dic=StandardDeARMatching(source,target,geod,F0,a0,a1,b1,c1,d1,a2,params)
        #geod, ener, Dic = SymmetricDeARMatching(source,target,geod,F0,a0,a1,b1,c1,d1,a2,params)
        print(j,F0.shape)
        if time_steps>2:
            geod=DeARMidpoint(geod,time_steps,F0,a0,a1,b1,c1,d1,a2,params)
    print(iterations,F0.shape)            
